...
public class AttackState : IEnemyState
{
    ...
    public void Update(EnemyManager m)
    {
        ...
        // La recarga se maneja como estado propio para que aparezca correctamente
        // en el debug/Inspector y no quede mezclada dentro de AttackState.
        if (shooter != null && shooter.useAmmo && (shooter.IsReloading || shooter.IsMagazineEmpty))
        {
            m.GoToReload();
            return;
        }
        ...
        if (canSee)
        {
            bool canShootNow = shooter != null && Time.time >= m.ShootBlockedUntil;
    ...
    bool TryMoveCapsuleWithSlide(EnemyManager m, Vector3 delta, float minKeepDistanceToTarget, out bool moved)
    {
        ...
        if (Physics.CapsuleCast(p1, p2, castRadius, dir, out RaycastHit hit, dist + skin, m.combatObstacleMask, QueryTriggerInteraction.Ignore))
        {
            ...
            if (slide.sqrMagnitude > 1e-6f)
            {
                ...
                if (!Physics.CapsuleCast(p1, p2, castRadius, sDir, out RaycastHit hit2, sDist + skin, m.combatObstacleMask, QueryTriggerInteraction.Ignore))
                {
